...
`a5e34ff` login implementation on that same baseline, before integrating concurrent
main changes (`bab560f`, Friends cache and rustls update). Windows 11 Home 10.0.26200,
...
the shared Cargo target was serialized. The isolated comparison includes no dependency or
lockfile change; it is not a comparison of the later integrated package against old main.
...
unexpected process termination and teardown. The isolated-change runs passed with normal plugins
...
After integrating main `bab560f`, all eight focused Linux checks and strict Clippy passed
again; the real WebKit test also passed with normal plugins (1.38 s) and empty plugins
