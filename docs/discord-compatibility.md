...
Navigation is limited to Discord’s HTTPS origin; new windows and downloads are blocked. Third-party challenge subframes are left to the platform engine; popup-dependent methods may fail. Linux login advertises the shared Chrome user agent; experimental JavaScript vendor/Chrome-namespace overrides were removed after unsuccessful owner testing. QR compatibility remains unverified. Resume URLs are restricted to recognized Discord gateway hosts.
