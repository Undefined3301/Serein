...
`messages.read` refers to local RPC; it is not a general REST chat grant. `rpc` needs approved-partner access and depends on the official application. Social SDK relationship access does not establish general desktop-client chat access. No client secret, token broker, native password endpoint, CAPTCHA/MFA workaround, or harvested QR flow is implemented. Browser-identity overrides are documented in the [authentication guide](authentication.md); they do not establish service acceptance. The owner explicitly requested the official login page in an authentication-only webview and secure saved login; OAuth is not the product login.
...
Navigation is limited to Discord’s HTTPS origin; new windows and downloads are blocked. Third-party challenge subframes are left to the platform engine; popup-dependent methods may fail. Linux login advertises the shared Chrome user agent and overrides two top-frame identity hints (vendor and an absent Chrome namespace). This is not Chromium emulation and can select unsupported page code; QR compatibility remains unverified. Resume URLs are restricted to recognized Discord gateway hosts.
